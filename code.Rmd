...
###4 Result
##4.1 Compariosn of different models
```{r result}
result <- data.frame(training_accuracy =c(0.853125,0.90938,0.8828,0.821875,0.834375),test_accuracy= c(0.725,0.7125,0.725,0.80625,0.83125),method = c("glmnet","tree","tree_prune","bagging","random_forest"))
#plot
ggplot(result) +
        geom_point(aes(x= method, y = training_accuracy,color = "training"))+
        geom_line(aes(x= method, y = training_accuracy,color = "training",group = 1)) +
        geom_point(aes(x= method,y = test_accuracy,color = "test")) + 
        geom_line(aes(x= method,y = test_accuracy,color = "test",group=1)) +
        labs(y = "prediction accuracy", title = "Performance of 5 methods" ) + 
        theme(plot.title = element_text(size = 18),axis.title = element_text(size = 15),axis.text = element_text(size = 12),legend.text = element_text(size = 15),legend.title = element_text(size = 18))
